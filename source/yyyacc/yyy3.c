...
stagen(){ /* generate the states */
  ...
  for( i=0; i<nstate; ++i ){
    ...
    if( gsdebug ){
      ...
      fprintf(Fout,  "%d: ", i );
      for( j=1; j<=nterms; ++j ){
        if( temp1[j] != 0 ) fprintf(Fout,  "%s %d, ", symnam(j), temp1[j]);
        }
      for( j=1; j<=nnonter; ++j ){
        if( temp1[j+nterms] ) fprintf(Fout,  "%s %d, ", nontrst[j].name, temp1[j+nterms] );
        }
      fprintf(Fout, "\n");
      }
...
closure(i){ /* generate the closure of state i */
    ...
    for( j=0; j<cwset; ++j ){
      ...
      for( k=j; k<cwset; ++k ){
        if( wsets[k].flag == 1 && *(pi=wsets[k].pitem) == c ){
          ...
          if( ch<=0 ) sunion( clset.lset, clset.lset, wsets[k].ws );
          }
        }
      ...
      for( s=pres[c]; s<t; ++s ){
        ...
        for( k=0; k<cwset; ++k ){ /* is the item there */
          if( wsets[k].pitem == *s ){ /* yes, it is there */
            ...
            if( sunion( wsets[k].ws, wsets[k].ws, clset.lset ) ) wsets[k].flag = work = 1;
            goto nexts;
            }
          }
  ...
  fprintf(Fout, "\nState %d, nolook = %d\n", i, nolook );
  for( j=0; j<cwset; ++j ){
    if( wsets[j].flag ) fprintf(Fout, "flag set!\n");
    wsets[j].flag = 0;
