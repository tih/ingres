...
/*
** these two are used somewhere in some ingres library
	...
	while (read(dfd, d, 16) == 16)
	{
		...
		if (bequal(MODBATCH, d->fname, sizeof(MODBATCH) - 1))
		{
			...
			printf("\tFound incomplete modify of %.12s, user = %s\n",
				descr.relid, lookucode(descr.relowner));

			bmove(&descr.relowner, Usercode, sizeof(descr.relowner));
			close(Batch_fp);
			if (ask("\tComplete? "))
				modupdate();
		}
...
checkatts()
{
	...
	register int		i;
	register int		once;
	struct tup_id		tid, limtid, reltid;
	char			key[MAXTUP];
